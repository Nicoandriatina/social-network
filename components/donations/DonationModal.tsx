...
import { 
  ...
  DollarSign, Package, ShoppingCart, Target, TrendingUp
} from 'lucide-react';
import { AvatarDisplay } from '../AvatarDisplay';
...
const DonationModal = ({ isOpen, onClose, onSuccess }) => {
  ...
  // ✅ NOUVEAU : État pour les besoins du projet sélectionné
  ...
  // ✅ NOUVEAU : Charger les besoins quand un projet est sélectionné
  useEffect(() => {
    if (formData.destinationType === 'project' && formData.destinationId) {
      ...
      if (selectedProject && selectedProject.besoins && selectedProject.besoins.length > 0) {
  ...
  // ✅ NOUVEAU : Adapter le type de don selon le besoin sélectionné
  useEffect(() => {
  ...
  const validateForm = () => {
    ...
    // ✅ Si un projet avec besoins, la sélection d'un besoin est optionnelle
    // mais si sélectionné, on vérifie la cohérence
  ...
  const handleSubmit = async () => {
    ...
    try {
      const donationData = {
        ...
        needId: formData.needId || undefined, // ✅ NOUVEAU
        ...(formData.destinationType === 'project' && { projectId: formData.destinationId }),
        ...(formData.destinationType === 'etablissement' && { etablissementId: formData.destinationId }),
        ...(formData.destinationType === 'personnel' && { personnelId: formData.destinationId }),
      };
    ...
    <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50 overflow-y-auto">
      <div className="bg-white rounded-2xl w-full max-w-3xl my-8 max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-6 space-y-6">
          ...
          {/* ✅ NOUVEAU : Sélection du besoin spécifique (si projet avec besoins) */}
{showNeedsSelection && projectNeeds.length > 0 && (
            <div className="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-xl p-5 border-2 border-blue-200">
              ...
              <p className="text-sm text-slate-600 mb-4">
                Ce projet a {projectNeeds.length} besoin{projectNeeds.length > 1 ? 's' : ''} spécifique{projectNeeds.length > 1 ? 's' : ''}. 
                Vous pouvez choisir de contribuer à un besoin particulier ou faire un don général au projet.
              </p>
              
              <div className="space-y-3">
                {/* Option: Don général (pas de besoin spécifique) */}
                <button
                  type="button"
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-slate-500">Vous choisissez de soutenir le projet dans son ensemble</p>
                    </div>
                  </div>
                </button>
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="space-y-1">
                            ...
                            <div className="flex items-center justify-between text-xs text-slate-600">
                              {need.type === 'MONETAIRE' ? (
                                <>
                                  <span>{formatNumber(need.montantRecu || 0)} Ar</span>
                                  <span>sur {formatNumber(need.montantCible || 0)} Ar</span>
                                </>
                              ) : (
                                <>
                                  <span>{need.quantiteRecue || 0} {need.unite}</span>
                                  <span>sur {need.quantiteCible || 0} {need.unite}</span>
                                </>
                              )}
                            </div>
                          </div>
          ...
          {/* Type de don - Désactivé si besoin spécifique sélectionné */}
          <div>
            <label className="block text-sm font-semibold text-slate-700 mb-3">
              Type de don *
